...
public class TwosixWhiteboardLink extends Link {
    ...
    void twosixWhiteboardMonitor(double timestamp) {
        ...
        if (lastTimestamp < 0) {
            lastTimestamp = readValue("lastTimestamp", Time.getTimestampSeconds());
        }
        int latest = getIndexFromTimestamp(lastTimestamp);

        // TODO: Once there is a way to inform NM, limit retries on failure. break and
        // notify NM if
